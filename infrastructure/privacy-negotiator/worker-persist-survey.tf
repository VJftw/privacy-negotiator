...
resource "aws_cloudwatch_log_metric_filter" "worker-persist-survey_error" {

  name = "${var.environment}-worker-persist-survey.error"
  pattern = "error"
  log_group_name = "${aws_cloudwatch_log_group.worker-persist-survey.name}"

  metric_transformation {
    name = "${var.environment}-worker-persist-survey.error"
    namespace = "${var.environment}-worker-persist-survey"
    value = "1"
  }

}

resource "aws_cloudwatch_log_metric_filter" "worker-persist-survey_error_reset" {
...
resource "aws_cloudwatch_metric_alarm" "worker-persist-survey_error" {

  alarm_name = "${var.environment}.worker-persist-survey.error"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  threshold = "1"
  period = "60"
  statistic = "Sum"
  evaluation_periods = "1"
  metric_name = "${var.environment}.worker-persist-survey.error"
  namespace = "${var.environment}-worker-persist-survey.error"
  alarm_description = "monitors log for worker-persist-survey errors"
